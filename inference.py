...
speaker_vocab_dict_path = 'vocabs_iemocap/speaker_vocab.pkl'
emotion_vocab_dict_path = 'vocabs_iemocap/emotion_vocab.pkl'
sentiment_vocab_dict_path = 'vocabs_iemocap/sentiment_vocab.pkl'
...
# IEMOCAP data loader  
def load_iemocap_and_builddataset(dialogues, emotions, speakers):
...
def train(model, train_data_path, dev_data_path, test_data_path):
    ...
    if CONFIG['task_name'] == 'iemocap':
        ...
        devset = load_iemocap_and_builddataset(dialogues_dev, emotions_dev, speakers_dev)
        testset = load_iemocap_and_builddataset(dialogues_test, emotions_test, speakers_test)
        trainset = load_iemocap_and_builddataset(dialogues_train, emotions_train, speakers_train)
    else:
        ...
        for batch_idx in range(mask.shape[0]):
            for seq_idx in range(mask.shape[1]):
                if mask[batch_idx][seq_idx]:
                    ...
                    # pred_list.append([sentences[0], outputs[batch_idx][seq_idx]])
                    # y_true_list.append(emotion_idxs[batch_idx][seq_idx])
        tq_test.update()
...
if __name__ == '__main__':
    ...
    parser.add_argument('-tsk', '--task_name', default='iemocap', type=str)
    ...
    get_vocabs(dialogues_test, emotions_test, speakers_test)
    ...
    if args.finetune:
        ...
        print('checkpoint {} is loaded'.format(model_path))

    # if args.train:
    #     train(model, train_data_path, dev_data_path, test_data_path)
    # if args.test:
    #     # testset = load_iemocap_and_builddataset(dev_data_path)
    #     if args.task_name =='emorynlp':
    #         testset = load_emorynlp_and_builddataset(test_data_path)
    #     if args.task_name == 'iemocap':
    #         dialogues_test, emotions_test, speakers_test = data_reader(test_data_path)
    ...
    if args.task_name =='iemocap':
            testset = load_iemocap_and_builddataset(dialogues_test, emotions_test, speakers_test)
    # testset = load_iemocap_and_builddataset(dialogues_test, emotions_test, speakers_test)
    result = inference(model, testset)

    print('-'*25, 'RESULT', '-'*25)
    print(result)

    # utterances = [y for x in dialogues_test for y in x]
    # print(utterances)
